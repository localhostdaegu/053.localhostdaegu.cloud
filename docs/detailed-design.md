...
<div class="notice"><strong>기록 기준</strong><br>9월 18일 서비스 개발 작업 로그의 사전상담 전환(T1~T7), 지역 매칭·노트 저장, 미입력·연도 전달·문서 저장·세션 갱신 후속 작업을 반영했습니다. 코드 프리즈는 해제됐으며 배포 환경에서는 아직 확인하지 않았습니다.</div>
...
| `finance_product` · `finance_product_category` · `product_consultation_metadata` · `product_procedure_step` | 상품·업종 조건·상담 메타데이터·절차 | 상품 12건, 9/18 원문 대조 12/12. 은행 연결 등급 direct 2·linked 3·unverified 7 |
| `external_dataset` · `regional_indicator` | 외부 데이터셋·지역 지표 | 테이블만 준비. 센터 D1·D2 미신청·미확보로 0행 |
...
**스키마·경계 후속 작업.** 19 → 29테이블 확장 마이그레이션(`b93358fab70e`)은 기존 컬럼 변경 없이 테이블 10개·인덱스 8개를 추가했고 테스트 DB 왕복·`alembic check`를 통과했습니다. 이후 지역 한정 상품용 `district_code`를 별도 마이그레이션(`079cb96619ca`)으로 추가했습니다. 개발 DB 상품 재시드와 상담 세션·계획 생성도 후속 로그에서 확인했습니다. 29테이블 ERD 원문은 서비스 저장소 `docs/erd.md`에 있습니다.
...
| 구분 | 현재 설정 | 확인 근거 |
|---|---|---|
| 모델 설정 | 기본 리포트 `gemini-3.8-flash`, 임베딩 `gemini-embedding-001`. 리포트는 `REPORT_WRITER_PROVIDER=ollama`·`OLLAMA_REPORT_MODEL=gemma4:12b`로 로컬 선택 가능. qwen@2560 검색 전환은 컬럼 변경·재색인·provider 설정화 필요 | 9/18 [모델 평가]({{ '/docs/model-evaluation/' | relative_url }}) |
