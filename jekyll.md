...
### 제안요약서 — 타깃 업종 10종의 선정 근거 복원

Metabole 시절 브레인스토밍(수요 동인 유형 4축: 일상소비·여가·경기민감·인구구조)을 대구 기준으로 다시 써 `docs/plan/plan.md` §1-4에 넣었다. 원형 3종(카페 기준 모델·노래방 정책 충격 실험군·부동산 반례)과 확장 7종, 교차 시그널을 설명하되 **대구 프로토타입에서 지표가 있는 건 인허가 7종뿐**(편의점·부동산·학원·어린이집은 등록만, 수집 전)임을 명시했다. `industry.demand_type` 컬럼이 이 4유형을 코드화하고 있다. 허브에도 같은 내용을 `docs/target-industries.md`로 올렸다.

### 운영 임베딩 차원 2560 통일 — 오프라인 전환을 수 분 작업으로

- **결정(사용자)**: 운영은 Gemini 유지, 오프라인은 시연 대비 배선. 전환이 수 분이 되려면 차원이 같아야 하므로 Gemini도 로컬 qwen과 같은 **2560**으로.
- **실측 결함**: `vector(2560)`으로 바꾸자 HNSW 인덱스 생성이 `column cannot have more than 2000 dimensions for hnsw index`로 실패 — pgvector `vector` 타입 인덱스 상한. **`halfvec(2560)`**(fp16, 4000차원까지)으로 마이그레이션 `c1d2e3f4a5b6`. 기존 벡터는 비우고 Gemini 2560으로 전량 재색인(3,589청크, 2분 33초).
- **설정화**: `RAG_EMBEDDING_PROVIDER`(검색 임베더, 기본 gemini) 추가. 작성기는 어제 `REPORT_WRITER_PROVIDER`. 오프라인 전환 = 재색인 3분 + 환경변수 2개(`docs/model-evaluation.md` §11).
- **검증**: `alembic check` 통과, 백엔드 497 passed. 재색인 후 평가셋 Recall@5 0.438은 정답 공고(2025년)가 운영 만료 필터에 걸린 탓 — 필터 OFF면 funding 20건 Recall 1.000·MRR 0.91. 평가셋 검수 때 미만료 공고로 재추출 필요.
- **부수 수정**: 테스트 픽스처의 1536 상수 → 2560, 스키마 테스트 halfvec 확인으로 교체.

### 제안요약서 문제정의 출처 보강

`docs/plan/plan.md` §1-1에 "왜 사회적 문제인가 — 규모와 출처" 표를 추가했다. 2026-09-19 원문 확인: 2025년 폐업 97.6만·소상공인 6대 업종 폐업률 11.08%(국세청/서울경제), 5년 생존율 36.4%(국가데이터처 기업생멸행정통계), 폐업 소상공인 업력 5년 미만 56.5%·폐업 시 부채 1억 236만(중기중앙회), 창업비용 8,300만 중 본인부담 5,900만(중기부 소상공인실태조사), 취약 자영업자 연체율 12.24%(한은). 대구: 2023년 외식업 폐업률 21.71% 전국 최고(핀다), 2024년 순창업 −2,287·5년 생존율 47.0%(동북지방통계청). 부록 5-8에 출처 8건 목록. 상품 등급 분포도 2·7·3으로 갱신.

### 미결 항목 정리 — 사용자 결정 6건과 재단 상품 승격

- **OPEN-004** 센터(카드·생활인구) 데이터는 대구 방문이 필요해 본선 진출 시 9/28 멘토링 당일 방문으로 계획. **OPEN-008** 카페 단기 폐업(개업 30일 이내 종료)은 폐업률·위험도에 **미반영**하기로 결정 — 집계 코드 반영은 미착수. **OPEN-009** 위험 점수는 **지역 간 상대 순위**로 표현 — 표기 문구 미착수. **OPEN-011** 오프라인 전환은 평가(9/18) 완료, 전환 시점만 유지. **OPEN-013** youth-1 2026 재공고는 미확인 유지.
- **OPEN-012 해소 — 재단 보증 4건 `unverified` → `linked`**: 재단 보증서 담보 대출은 특정 은행에 묶이지 않는 전 은행 취급 구조(사용자 확인). 원문의 "시중은행" 표기는 제한이 아니라는 뜻이므로 dgsinbo-1~4를 승격. `data/manual/dgsinbo_products.json` 수정 후 시드 재실행, DB 분포 **direct 2 · linked 7 · unverified 3**(youth-2~4). iM뱅크 상담 후보 3→7건 — 특히 사업자등록 전 신청 가능한 「유망 예비창업자 사전보증」이 후보 1군으로 올라온다. 근거가 원문 명시가 아니라 제도 구조에 대한 팀 확인이라는 점을 조사 문서에 남겼다. 백엔드 테스트 전체 통과.

### 데이터 — 공공데이터 5종(6파일) 다운로드·행정동 지표 적재

DEC-016은 마감 전 전통시장·나들가게만 여력 시였는데, 사용자 결정으로 받은 것 전부를 적용했다. 새 테이블 없이 어제 만든 `external_dataset`(출처·산식·제한)·`regional_indicator`(행정동 long format)에 넣었다. 파일·출처 `data/raw/daegu_extra/MANIFEST.md`.

- **출처**: 전부 공공데이터포털 파일(로그인·키 불요). 전통시장현황 15052837 · 나들가게 GPS 15122564 · 온누리 가맹점 3060079 · 백년가게 15132695 · 역별 시간별 승하차 15002503 · 월별 승차 15060371 + 역 주소 15041102. 전통시장 좌표는 브이월드 `LT_P_TRADSIJANG`(107곳).
- **로더** `apps/indicator/adapter/inbound/cli/load_open_data.py`: 데이터셋별 Strategy 5개. 점→행정동은 `RegionIndex`를 store CLI에서 `core/matrix/grid_geo_region_index.py`로 옮겨 공유(store·tobacco import 교체). 지오코딩 결과는 CSV 캐시. 재실행은 데이터셋 단위 교체(delete→upsert).
- **실측 결함 2건**: ① 브이월드 지오코더가 "달구벌대로 **지하** 2672" 같은 지하 주소를 전부 같은 지점(신매동)으로 돌려줌 → 역 7곳이 같은 좌표. '지하' 제거 후 호출하니 94/94 고유 좌표. ② 승하차 파일은 옛 역명(대공원·어린이회관), 역 주소 파일은 현 역명(수성알파시티·어린이세상) → 별칭 매핑.
- **적재 결과**: 전통시장 107곳→66개 동 / 나들가게 513→492곳 판정, 125개 동 / 백년가게 83→81곳, 46개 동 / 온누리 가맹점 12,430 중 **시장명 매칭 7,085(57%)** 만 64개 동(원천에 주소가 시도 단위뿐이라 시장명으로만 귀속 — 제한사항에 기록) / 승차 월별 1,064행·시간대별 1,568행(56개 동, 경산 5역 제외).
- **하지 않은 것**: 화면·리포트 노출은 미배선 — 지표는 DB에만 있다. "전통시장 반경 500m" 파생변수 미구현. 온누리 미매칭 43%는 골목형상점가·상가명이라 별도 매핑 필요.
- **테스트**: 백엔드 전체 통과(신규 `tests/test_indicator_open_data.py` 5건).
